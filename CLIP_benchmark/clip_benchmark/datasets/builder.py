...
def _download_to(url, root, filename):
    os.makedirs(root, exist_ok=True)
    download_url(url, root=root, filename=filename)


def _write_coco_karpathy_annotation(karpathy_file, split, annotation_file):
    ...
    for entry in data["images"]:
        ...
        image_id = int(entry.get("cocoid", entry["imgid"]))
    ...
    if not images:
        raise ValueError(f"No COCO Karpathy entries found for split '{split}' in {karpathy_file}")
...
def build_dataset(dataset_name, root="root", transform=None, split="test", download=True, annotation_file=None, language="en", task="zeroshot_classification", wds_cache_dir=None, custom_classname_file=None, custom_template_file=None, **kwargs):
    ...
    if dataset_name == "cifar10":
        ...
        if not os.path.exists(root_split):
            ...
            if not os.path.exists(os.path.join(root, archive_name)):
                _download_to(
                    f"http://images.cocodataset.org/zips/{archive_name}",
                    root,
                    archive_name,
                )
            extract_archive(os.path.join(root, archive_name), root)
        if not annotation_file:
            annotation_file = f"{root}/coco_{split}_karpathy.json"
        if not os.path.exists(annotation_file):
            karpathy_file = os.path.join(root, "karpathy", "dataset_coco.json")
            if os.path.exists(karpathy_file):
                _write_coco_karpathy_annotation(karpathy_file, split, annotation_file)
            else:
                _download_to(
                    f"https://github.com/mehdidc/retrieval_annotations/releases/download/1.0.0/coco_{split}_karpathy.json",
                    root,
